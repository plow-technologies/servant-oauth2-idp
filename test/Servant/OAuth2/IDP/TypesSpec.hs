...
import Servant.OAuth2.IDP.Types (AccessToken (..), LoginAction (..), OAuthGrantType (..), RefreshToken (..), Scope (..), Scopes (..), TokenType (..), TokenValidity, mkClientName, mkClientSecret, mkRedirectUri, mkScope, mkTokenValidity, parseScopes, serializeScopeSet, unAccessToken, unClientName, unClientSecret, unRefreshToken, unTokenType)
import Test.Hspec
...
spec = do
  ...
  describe "FR-004c: TokenValidity newtype" $ do
    ...
    context "FromJSON instance (OAuth wire format parsing)" $ do
      -- \| FR-1: FromJSON TokenValidity parses integer seconds
      -- \| bead all-wvs.16
      it "parses integer seconds" $
        let parsed = decode "3600" :: Maybe TokenValidity
            expected = Just (mkTokenValidity 3600)
         in parsed `shouldBe` expected

      -- \| FR-2: JSON roundtrip: decode . encode == id
      -- \| bead all-wvs.16
      it "roundtrips 3600 seconds through encode/decode" $
        let validity = mkTokenValidity 3600
            encoded = encode validity
            parsed = decode encoded :: Maybe TokenValidity
         in parsed `shouldBe` Just validity

      -- \| FR-3: Negative values parse successfully
      -- \| bead all-wvs.16
      it "parses negative seconds" $
        let parsed = decode "-60" :: Maybe TokenValidity
            expected = Just (mkTokenValidity (-60))
         in parsed `shouldBe` expected

      -- \| EDGE-1: Zero seconds parses
      -- \| bead all-wvs.16
      it "parses zero seconds" $
        let parsed = decode "0" :: Maybe TokenValidity
            expected = Just (mkTokenValidity 0)
         in parsed `shouldBe` expected

      -- \| EDGE-2: Fractional values floor to integer
      -- \| bead all-wvs.16
      it "roundtrips fractional seconds (floors to integer)" $
        let validity = mkTokenValidity 3600.5
            encoded = encode validity
            decoded = decode encoded :: Maybe TokenValidity
         in decoded `shouldBe` Just (mkTokenValidity 3600)
